                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($opdPatients as $patient)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                @php
                                    $canSendToDoctor = $opdExam && $medicalChecklist;
                                @endphp
                                @if($canSendToDoctor)
                                    <form action="{{ route('nurse.opd.send-to-doctor', $patient->id) }}" method="POST" class="inline">
                                        @csrf
                                ...
                                @endif
                                @if($opdExam)
                                    <a href="{{ route('nurse.opd.edit', $opdExam->id) }}" class="bg-green-600 text-white px-3 py-1 rounded hover:bg-green-700 transition-colors mr-2" title="Edit Examination">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                @else
                                    <a href="{{ route('nurse.opd.create', ['user_id' => $patient->id]) }}" class="bg-green-600 text-white px-3 py-1 rounded hover:bg-green-700 transition-colors mr-2" title="Create Examination">
                                        <i class="fas fa-plus"></i>
                                    </a>
                                @endif
                                <a href="{{ route('nurse.medical-checklist.opd', $patient->id) }}" class="bg-blue-600 text-white px-3 py-1 rounded hover:bg-blue-700 transition-colors mr-2" title="Medical Checklist">
