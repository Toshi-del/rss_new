...
@section('title', 'Sent Annual Physical Examination - RSS Citi Health Services')
    ...
    <div class="px-4 py-6 sm:px-0">
        <div class="mb-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-semibold text-gray-900">Annual Physical Examination Results</h1>
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            ...
            <div class="px-6 py-4 bg-blue-50 border-b border-gray-200">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-sm text-gray-600">Patient ID: {{ $examination->patient_id }}</p>
                    </div>
            ...
            <div class="px-6 py-4">
                ...
                <div class="mt-8">
                    <h3 class="text-lg font-medium text-gray-900 mb-4">Physical Findings</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach($examination->physical_findings as $area => $finding)
                        <div class="bg-gray-50 p-4 rounded-lg">
                            <div class="flex items-center justify-between mb-2">
                                <h4 class="font-medium text-gray-900">{{ $area }}</h4>
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $finding['result'] === 'Normal' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                                    {{ $finding['result'] }}
                                </span>
                            </div>
                            @if(isset($finding['findings']))
                                <p class="text-sm text-gray-600">{{ $finding['findings'] }}</p>
                            @endif
                        </div>
                        @endforeach
                    </div>
                ...
                <div class="mt-8">
                    <h3 class="text-lg font-medium text-gray-900 mb-4">Laboratory Findings</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach($examination->lab_findings as $test => $result)
                        <div class="bg-gray-50 p-4 rounded-lg">
                            <div class="flex items-center justify-between mb-2">
                                <h4 class="font-medium text-gray-900">{{ $test }}</h4>
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $result['result'] === 'Normal' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                                    {{ $result['result'] }}
                                </span>
                            </div>
