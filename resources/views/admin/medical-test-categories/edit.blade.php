...
@section('page-title', 'Edit Medical Test Category')
...
<div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 p-6">
    <div class="max-w-5xl mx-auto space-y-8">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="flex flex-wrap items-center gap-3">
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold {{ $category->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                <i class="fas fa-{{ $category->is_active ? 'check-circle' : 'times-circle' }} mr-1.5 text-xs"></i>
                                {{ $category->is_active ? 'Active' : 'Inactive' }}
                            </span>
                            <div class="flex items-center space-x-1 text-sm text-gray-600">
                                <i class="fas fa-vial text-gray-400"></i>
                                <span>{{ $category->medical_tests_count ?? 0 }} tests</span>
                            </div>
                            <div class="flex items-center space-x-1 text-sm text-gray-600">
            ...
            <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20 p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-sm font-medium text-gray-900">{{ $category->medical_tests_count ?? 0 }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 overflow-hidden">
            ...
            <form action="{{ route('admin.medical-test-categories.update', $category) }}" method="POST" class="p-8 space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" 
                           ...
                           class="w-full px-4 py-3 border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all duration-150 @error('name') border-red-500 ring-2 ring-red-200 @enderror" 
                           required>
                    @error('name')
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="description" 
                              ...
                              class="w-full px-4 py-3 border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all duration-150 resize-none @error('description') border-red-500 ring-2 ring-red-200 @enderror">{{ old('description', $category->description) }}</textarea>
                    @error('description')
                        <div class="flex items-center space-x-2 mt-2">
                            <i class="fas fa-exclamation-circle text-red-500 text-sm"></i>
                            <p class="text-sm text-red-600">{{ $message }}</p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="number" 
                               ...
                               min="0"
                               placeholder="0"
                               class="w-full px-4 py-3 border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all duration-150 @error('sort_order') border-red-500 ring-2 ring-red-200 @enderror">
