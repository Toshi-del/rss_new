...
@section('page-title', 'Pre-Employment Record Details')
    ...
    <div class="bg-gradient-to-r from-teal-600 to-blue-600 rounded-xl shadow-lg p-6 text-white mb-8">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-teal-100">{{ $preEmployment->full_name }}</p>
                <p class="text-sm text-teal-200">Record ID: #{{ $preEmployment->id }}</p>
            </div>
            <div class="text-right">
                <div class="flex items-center space-x-3">
                    ...
                    @endphp
                    <span class="px-4 py-2 rounded-full text-sm font-bold text-white {{ $config[0] }} flex items-center">
                        <i class="{{ $config[1] }} mr-2"></i>
                        {{ $config[2] }}
                    </span>
                </div>
                <p class="text-sm text-teal-200 mt-2">Created: {{ $preEmployment->created_at->format('M d, Y') }}</p>
            </div>
        </div>
    </div>
        ...
        <div class="lg:col-span-2 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-4">
                        <div>
                            ...
                            <p class="text-lg font-semibold text-gray-900">{{ $preEmployment->full_name }}</p>
                        </div>
                        
                        <div>
                            <label class="block text-sm font-medium text-gray-600">Age</label>
                            <p class="text-gray-900">{{ $preEmployment->age }} years old</p>
                        </div>
                        
                        <div>
                            <label class="block text-sm font-medium text-gray-600">Sex</label>
                            <span class="px-3 py-1 text-sm font-medium rounded-full {{ $preEmployment->sex === 'Male' ? 'bg-blue-100 text-blue-800' : 'bg-pink-100 text-pink-800' }}">
                                {{ $preEmployment->sex }}
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <p class="text-gray-900">{{ $preEmployment->email }}</p>
                        </div>
                        
                        <div>
                            <label class="block text-sm font-medium text-gray-600">Phone Number</label>
                            <p class="text-gray-900">{{ $preEmployment->phone_number ?? 'Not provided' }}</p>
                        </div>
            ...
            @if($pathologistTests->isNotEmpty())
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                                ...
                                @foreach($tests as $test)
                                    <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                                        <div class="flex items-start justify-between">
                                            <div class="flex-1">
                                                <h5 class="font-semibold text-gray-900">{{ $test['test_name'] }}</h5>
                                                @if(isset($test['category_name']))
                                                    <p class="text-sm text-gray-600 mt-1">Category: {{ $test['category_name'] }}</p>
                                                @endif
                    ...
                    <!-- Total Price -->
                    @if($preEmployment->total_price > 0)
                        <div class="mt-6 pt-6 border-t border-gray-200">
                            <div class="flex items-center justify-between">
                                <span class="text-lg font-semibold text-gray-700">Total Price:</span>
                                <span class="text-2xl font-bold text-emerald-600">₱{{ number_format($preEmployment->total_price, 2) }}</span>
                            </div>
                        </div>
                    @endif
                </div>
                ...
                @if($allSelectedTests->isNotEmpty())
                    <div class="mt-8 bg-gray-50 rounded-lg p-4 border border-gray-200">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            @foreach($allSelectedTests as $originalTest)
                                <div class="flex items-center justify-between bg-white rounded-lg p-3 border border-gray-200">
                                    <div>
                                        <span class="font-medium text-gray-900">{{ $originalTest['test_name'] }}</span>
                                        <div class="text-xs text-gray-600">{{ $originalTest['category_name'] }}</div>
                                    </div>
                                    @if($originalTest['price'] > 0)
                                        <span class="text-sm font-bold text-emerald-600">₱{{ number_format($originalTest['price'], 2) }}</span>
                                    @endif
                                </div>
                            @endforeach
                        </div>
                ...
                @if($preEmployment->medicalTest)
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        ...
                        <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                            <div class="flex items-start justify-between">
                                <div class="flex-1">
                                    <h5 class="font-semibold text-gray-900">{{ $preEmployment->medicalTest->name }}</h5>
                                    @if($preEmployment->medicalTestCategory)
                                        <p class="text-sm text-gray-600 mt-1">Category: {{ $preEmployment->medicalTestCategory->name }}</p>
                                    @endif
                                </div>
                                @if($preEmployment->medicalTest->price > 0)
                                    <div class="ml-4">
                                        <span class="px-2 py-1 text-xs font-bold bg-emerald-100 text-emerald-700 rounded">
                                            ₱{{ number_format($preEmployment->medicalTest->price, 2) }}
                                        </span>
                                    </div>
                                @endif
                            </div>
            ...
            @if($preEmployment->other_exams && !empty($preEmployment->parsed_other_exams['additional_exams']))
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <h3 class="text-xl font-bold text-gray-800 mb-6">
                    ...
                    <div class="bg-blue-50 rounded-lg p-4 border border-blue-200">
                        ...
                        <p class="text-sm text-blue-700">{{ $preEmployment->parsed_other_exams['additional_exams'] }}</p>
                    </div>
                </div>
            @endif

            <!-- Laboratory Results -->
            @if($preEmployment->preEmploymentExamination && $preEmployment->preEmploymentExamination->lab_report)
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <h3 class="text-xl font-bold text-gray-800 mb-6">
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full border-collapse border border-gray-300">
                                ...
                                <tbody>
                                    ...
                                    @forelse($displayedTests as $testSlug => $testData)
                                        @if(!empty($testData['result']) && $testData['result'] !== 'Not available')
                                            <tr>
                                                ...
                                                <td class="border border-gray-300 px-4 py-3 text-sm text-gray-900">
                                                    {{ !empty($testData['findings']) ? $testData['findings'] : '-' }}
                                                </td>
                                            </tr>
                                        @endif
                                    @empty
                                    ...
                                    @if(isset($labReport['additional_notes']) && !empty($labReport['additional_notes']))
                                        <tr>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div class="space-y-3 text-sm">
                    <div class="flex justify-between">
                        ...
                        <span class="font-medium text-gray-900">#{{ $preEmployment->id }}</span>
                    </div>
                    
                    <div class="flex justify-between">
                        <span class="text-gray-600">Created:</span>
                        <span class="font-medium text-gray-900">{{ $preEmployment->created_at->format('M d, Y') }}</span>
                    </div>
                    
                    <div class="flex justify-between">
                        <span class="text-gray-600">Updated:</span>
                        <span class="font-medium text-gray-900">{{ $preEmployment->updated_at->format('M d, Y') }}</span>
                    </div>
                    
                    <div class="flex justify-between">
                        <span class="text-gray-600">Billing Type:</span>
                        <span class="font-medium text-gray-900">{{ $preEmployment->billing_type }}</span>
                    </div>
                    
                    @if($preEmployment->uploaded_file)
                        <div class="flex justify-between">
                            <span class="text-gray-600">Source File:</span>
                            <span class="font-medium text-gray-900 text-xs">{{ $preEmployment->uploaded_file }}</span>
                        </div>
                    @endif
                </div>
            </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="space-y-3">
                        <div class="flex justify-between">
                            ...
                            <span class="font-bold text-gray-900">{{ $pathologistTests->count() }}</span>
                        </div>
                        
                        <div class="flex justify-between">
                            <span class="text-gray-600">Categories:</span>
                            <span class="font-bold text-gray-900">{{ $groupedTests->count() }}</span>
                        </div>
                        
                        <div class="flex justify-between">
                            <span class="text-gray-600">Package Tests:</span>
                            <span class="font-bold text-blue-600">{{ $pathologistTests->where('is_package_component', true)->count() }}</span>
                        </div>
                        
                        <div class="flex justify-between">
                            <span class="text-gray-600">Individual Tests:</span>
                            <span class="font-bold text-emerald-600">{{ $pathologistTests->where('is_package_component', false)->count() }}</span>
                        </div>
                        
                        @if($preEmployment->total_price > 0)
                            <div class="flex justify-between pt-2 border-t border-gray-200">
                                <span class="text-gray-600">Total Cost:</span>
                                <span class="font-bold text-emerald-600">₱{{ number_format($preEmployment->total_price, 2) }}</span>
                            </div>
                        @endif
                    </div>
                </div>
