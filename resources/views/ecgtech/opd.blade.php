...
@section('title', 'OPD Walk-in ECG Patients')

@section('page-title', 'OPD Walk-in ECG Patients')
@section('page-description', 'Manage ECG examinations for outpatient department walk-in patients')
...
<div class="content-card rounded-xl overflow-hidden shadow-lg border border-gray-200 mb-8">
        ...
        <div class="flex items-center justify-between">
            <div class="flex space-x-1">
                <a href="{{ request()->fullUrlWithQuery(['ecg_status' => 'needs_attention']) }}" 
                   class="px-4 py-2 text-sm font-medium rounded-lg transition-colors duration-200 {{ $currentTab === 'needs_attention' ? 'bg-orange-600 text-white' : 'text-gray-600 hover:text-orange-600 hover:bg-orange-50' }}">
                ...
                <a href="{{ request()->fullUrlWithQuery(['ecg_status' => 'ecg_completed']) }}" 
                   class="px-4 py-2 text-sm font-medium rounded-lg transition-colors duration-200 {{ $currentTab === 'ecg_completed' ? 'bg-orange-600 text-white' : 'text-gray-600 hover:text-orange-600 hover:bg-orange-50' }}">
            ...
            <a href="{{ route('ecgtech.opd') }}" class="text-sm text-gray-500 hover:text-gray-700 font-medium">
    ...
    <!-- Additional Filters -->
    <div class="p-6">
        <form method="GET" action="{{ route('ecgtech.opd') }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="gender" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 text-sm">
            ...
            <div class="flex items-center justify-between pt-4 border-t border-gray-200">
                <div class="flex items-center space-x-4">
                    <button type="submit" class="bg-orange-600 hover:bg-orange-700 text-white px-6 py-2 rounded-lg font-medium transition-colors duration-200">
    ...
    <div class="px-8 py-6 border-b border-gray-100 bg-gradient-to-r from-orange-50 to-amber-50">
        <div class="flex items-center justify-between">
            ...
            <div class="text-right">
                <div class="text-2xl font-bold text-orange-600" id="patientCount">{{ $opdPatients->count() }}</div>
            ...
            <tbody class="bg-white divide-y divide-gray-100">
                @forelse($opdPatients as $patient)
                    ...
                    <tr class="hover:bg-gray-50 transition-colors duration-200 patient-row" 
                            ...
                            <div class="flex items-center space-x-3">
                                <div class="w-10 h-10 bg-gradient-to-br from-orange-500 to-orange-600 rounded-full flex items-center justify-center">
                                    <span class="text-white font-semibold text-sm">
                                        {{ strtoupper(substr($patient->fname, 0, 1) . substr($patient->lname, 0, 1)) }}
                                    </span>
                                </div>
                                <div>
                        ...
                        <td class="px-6 py-6 whitespace-nowrap">
                            <div class="text-sm font-medium text-gray-900">{{ $patient->created_at->format('M d, Y') }}</div>
                        ...
                        <td class="px-6 py-6 whitespace-nowrap text-sm">
                            <div class="flex items-center space-x-2">
                                ...
                                @else
                                    <a href="{{ route('ecgtech.opd.create', ['user_id' => $patient->id]) }}" class="inline-flex items-center px-3 py-2 bg-green-600 hover:bg-green-700 text-white text-xs font-semibold rounded-lg transition-colors duration-200 group" title="Add ECG Results">
                                        <i class="fas fa-plus mr-1 group-hover:rotate-90 transition-transform"></i>
                                        Add ECG
                                    </a>
                                @endif
                                
                                <a href="{{ route('ecgtech.medical-checklist.opd', $patient->id) }}" class="inline-flex items-center px-3 py-2 bg-purple-600 hover:bg-purple-700 text-white text-xs font-semibold rounded-lg transition-colors duration-200 group" title="Medical Checklist">
