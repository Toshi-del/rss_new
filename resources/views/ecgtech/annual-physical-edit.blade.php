...
<div class="max-w-4xl mx-auto py-8">
    <div class="bg-white rounded-xl shadow-lg p-8 border border-gray-200 mb-8">
        <div class="bg-green-900 text-white text-center py-3 rounded-t-lg mb-8">
            <h2 class="text-xl font-bold tracking-wide">ECG EXAMINATION - ANNUAL PHYSICAL</h2>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div>
                ...
                <div class="text-lg font-semibold">{{ $annualPhysical->patient->first_name }} {{ $annualPhysical->patient->last_name }}</div>
            </div>
            <div>
        ...
        <form action="{{ route('ecgtech.annual-physical.update', $annualPhysical->patient->id) }}" method="POST" class="space-y-8">
            ...
            <div class="bg-green-50 rounded-lg p-6 border border-green-200">
                ...
                <div class="mb-6">
                    ...
                    <textarea name="ecg" rows="4" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500" placeholder="Enter ECG examination results..." required>{{ old('ecg', $annualPhysical->ecg ?? '') }}</textarea>
                    @error('ecg')
            ...
            <div class="bg-blue-50 rounded-lg p-6 border border-blue-200">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div>
                        ...
                        <div class="bg-white p-2 rounded-lg border border-gray-300 text-sm text-gray-600">{{ $annualPhysical->physical_exam['temp'] ?? 'Not recorded' }}</div>
                    </div>
                    <div>
                        <label class="block text-xs font-semibold uppercase mb-1">Height</label>
                        <div class="bg-white p-2 rounded-lg border border-gray-300 text-sm text-gray-600">{{ $annualPhysical->physical_exam['height'] ?? 'Not recorded' }}</div>
                    </div>
                    <div>
                        <label class="block text-xs font-semibold uppercase mb-1">Heart Rate</label>
                        <input type="text" name="heart_rate" class="w-full p-2 border border-gray-300 rounded-lg text-sm focus:ring-green-500 focus:border-green-500" value="{{ old('heart_rate', $annualPhysical->physical_exam['heart_rate'] ?? '') }}" placeholder="Enter heart rate">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold uppercase mb-1">Weight</label>
                        <div class="bg-white p-2 rounded-lg border border-gray-300 text-sm text-gray-600">{{ $annualPhysical->physical_exam['weight'] ?? 'Not recorded' }}</div>
                    </div>
                </div>
               
            <!-- Action Buttons -->
            <div class="flex flex-col md:flex-row justify-between items-center mt-8 gap-6">
                <a href="{{ route('ecgtech.annual-physical') }}" class="bg-gray-500 text-white px-6 py-2 rounded-lg shadow hover:bg-gray-600 transition-colors font-semibold">
                    <i class="fas fa-arrow-left mr-2"></i>Back to Annual Physical
                </a>
