...
@section('title', 'ECG Technician Dashboard')

@section('page-title', 'ECG Technician Dashboard')
@section('page-description', 'Manage ECG examinations and cardiac monitoring for all patient types')
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 hover:shadow-lg transition-all duration-300 group">
        <div class="p-8">
            <div class="flex items-center justify-between mb-6">
                ...
                <span class="bg-blue-100 text-blue-700 text-sm font-bold px-3 py-1 rounded-full">{{ $preEmploymentCount }}</span>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 hover:shadow-lg transition-all duration-300 group">
        <div class="p-8">
            <div class="flex items-center justify-between mb-6">
                ...
                <span class="bg-green-100 text-green-700 text-sm font-bold px-3 py-1 rounded-full">{{ $patientCount }}</span>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 hover:shadow-lg transition-all duration-300 group">
        <div class="p-8">
            <div class="flex items-center justify-between mb-6">
                ...
                <span class="bg-purple-100 text-purple-700 text-sm font-bold px-3 py-1 rounded-full">{{ $ecgReportCount }}</span>
            </div>
            ...
            <tbody class="bg-white divide-y divide-gray-100">
                @forelse($preEmployments as $preEmployment)
                    <tr class="hover:bg-gray-50 transition-colors duration-200">
                        ...
                        <td class="px-6 py-6 whitespace-nowrap text-sm text-gray-600 font-medium">{{ $preEmployment->company_name }}</td>
                        ...
                        <td class="px-6 py-6 whitespace-nowrap text-sm">
                            <div class="flex items-center space-x-2">
                                ...
                                <a href="{{ route('ecgtech.medical-checklist-page.pre-employment', $preEmployment->id) }}" class="inline-flex items-center px-3 py-2 bg-green-600 hover:bg-green-700 text-white text-xs font-semibold rounded-lg transition-colors duration-200" title="ECG Checklist">
            ...
            <tbody class="bg-white divide-y divide-gray-100">
                @forelse($patients as $patient)
                    <tr class="hover:bg-gray-50 transition-colors duration-200">
                        ...
                        <td class="px-6 py-6 whitespace-nowrap text-sm text-gray-600 font-medium">{{ $patient->age }}</td>
                        <td class="px-6 py-6 whitespace-nowrap text-sm text-gray-600 font-medium">{{ $patient->sex }}</td>
                        <td class="px-6 py-6 whitespace-nowrap text-sm text-gray-600 font-medium">{{ $patient->email }}</td>
                        <td class="px-6 py-6 whitespace-nowrap text-sm">
                            <div class="flex items-center space-x-2">
                                ...
                                <a href="{{ route('ecgtech.medical-checklist-page.annual-physical', $patient->id) }}" class="inline-flex items-center px-3 py-2 bg-green-600 hover:bg-green-700 text-white text-xs font-semibold rounded-lg transition-colors duration-200" title="ECG Checklist">
                                    <i class="fas fa-heartbeat mr-1"></i>
                                    ECG Check
                                </a>
                            </div>
                        ...
                        <td class="px-8 py-6 whitespace-nowrap">
                            <div class="flex items-center space-x-3">
                                <div class="w-10 h-10 bg-gradient-to-br from-purple-500 to-purple-600 rounded-full flex items-center justify-center">
                                    <span class="text-white font-semibold text-sm">
                                        {{ strtoupper(substr($report->patient_name ?? 'N', 0, 1) . substr(explode(' ', $report->patient_name ?? 'A')[1] ?? 'A', 0, 1)) }}
                                    </span>
                                </div>
                                <div>
                                    <p class="text-sm font-semibold text-gray-900">{{ $report->patient_name ?? 'N/A' }}</p>
                                    <p class="text-xs text-gray-500">ECG Report</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-6 whitespace-nowrap text-sm text-gray-600 font-medium">
                            {{ $report->date ? \Carbon\Carbon::parse($report->date)->format('M d, Y') : 'N/A' }}
                        </td>
                        <td class="px-6 py-6 whitespace-nowrap text-sm text-gray-600 font-medium">
                            <span class="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-gray-100 text-gray-700">
                                {{ $report->ecg_result ?? 'Pending Analysis' }}
                            </span>
                        </td>
                        <td class="px-6 py-6 whitespace-nowrap">
                            @php
