...
<div class="bg-white border border-gray-200 rounded-xl overflow-hidden mb-4">
  <div class="px-4 sm:px-6 py-4 border-b border-gray-200 flex items-center justify-between">
    <div class="flex items-center">
      <a href="<?php echo e(route('opd.dashboard')); ?>" class="inline-flex items-center px-3 py-2 rounded-lg border border-gray-300 text-sm text-gray-700 hover:bg-gray-50 transition mr-3">
  ...
  <div class="p-4 sm:p-6">
    ...
    ?>
    <div id="categoryList">
      <?php $__empty_1 = true; $__currentLoopData = $categoryItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
        <div class="mb-6 category-card" data-cat-name="<?php echo e(strtolower($category->name)); ?>" data-cat-desc="<?php echo e(strtolower($category->description ?? '')); ?>">
          <div class="flex items-center justify-between mb-2">
            <div>
              <h3 class="text-sm font-semibold text-gray-900"><?php echo e($category->name); ?></h3>
              <?php if($category->description): ?>
                <p class="text-xs text-gray-500"><?php echo e($category->description); ?></p>
              <?php endif; ?>
            </div>
            <?php $uniqueTests = $category->medicalTests->unique(function($t){ return strtolower($t->name ?? ''); }); ?>
            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-700"><?php echo e($uniqueTests->count()); ?> tests</span>
          </div>

          <?php if($uniqueTests->count() > 0): ?>
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            <?php $__currentLoopData = $uniqueTests; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $test): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
              <div>
                <div class="border border-gray-200 rounded-lg p-3 h-full flex flex-col test-item" data-test-name="<?php echo e(strtolower($test->name)); ?>" data-test-desc="<?php echo e(strtolower($test->description ?? '')); ?>">
                  <div class="flex items-start justify-between">
                    <div class="flex items-center">
                      <div class="w-9 h-9 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center mr-2">
                        <i class="<?php echo e(opd_icon_for_test($test->name)); ?>"></i>
                      </div>
                      <div>
                        <div class="font-semibold text-sm text-gray-900"><?php echo e($test->name); ?></div>
                        <?php if($test->description): ?>
                          <div class="text-xs text-gray-500"><?php echo e(Str::limit($test->description, 80)); ?></div>
                        <?php endif; ?>
                      </div>
                    </div>
                    <?php if(!is_null($test->price)): ?>
                      <span class="text-xs font-semibold text-blue-700">₱<?php echo e(number_format($test->price, 2)); ?></span>
                    <?php endif; ?>
                  </div>
                  <div class="mt-2 pt-2 text-right">
                    <form method="POST" action="<?php echo e(route('opd.incoming-tests.add', $test->id)); ?>" class="opd-add-form inline">
                      <?php echo csrf_field(); ?>
    ...
    <?php if(method_exists($categories, 'total')): ?>
    <div class="flex items-center justify-between mt-4">
      ...
      <div class="text-sm">
        <?php echo e($categories->onEachSide(1)->links()); ?>
...
<?php echo $__env->make('layouts.opd', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\rss_new\resources\views/opd/medical-test-categories.blade.php ENDPATH**/ ?>